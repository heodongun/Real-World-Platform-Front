...
import { DashboardStats, LeaderboardEntry, Problem } from '@/lib/types';
...
const defaultHealth = { status: 'UNKNOWN' };

export const HomePageClient = () => {
  const [problems, setProblems] = useState<Problem[]>([]);
  const [stats, setStats] = useState<DashboardStats | null>(null);
  const [leaderboard, setLeaderboard] = useState<LeaderboardEntry[]>([]);
  const [health, setHealth] = useState<{ status: string }>(defaultHealth);

  useEffect(() => {
    let active = true;
    const load = async () => {
      const [problemData, statData, leaderboardData, healthData] = await Promise.all([
        fetchProblems().catch(() => [] as Problem[]),
        fetchDashboardStats().catch(() => null),
        fetchLeaderboard().catch(() => [] as LeaderboardEntry[]),
        fetchHealth().catch(() => defaultHealth),
      ]);
      if (!active) return;
      setProblems(problemData);
      setStats(statData);
      setLeaderboard(leaderboardData);
      setHealth(healthData ?? defaultHealth);
    };
    load();
    return () => {
      active = false;

...
export const DashboardPageClient = () => {
  ...
  const [health, setHealth] = useState<{ status: string }>({ status: 'UNREACHABLE' });

  useEffect(() => {
    let active = true;
    const load = async () => {
      const [statsData, leaderboardData, healthData] = await Promise.all([
      ...
      setHealth(healthData ?? { status: 'UNREACHABLE' });
    };
    load();
    return () => {
      active = false;
